...
dds <- DESeqDataSetFromTximport(txi, colData = meta_df, design = ~ stage)
...
gsea_tbl <- gsea_res[, c("Description","NES","p.adjust","setSize","core_enrichment")]
head(gsea_tbl, 10)
...
# to pick 2 GO terms one with positive NES and one with negative NES
...
# finding the leading edge gene in each 
pick_core <- function(term_id, gsea_res_df, res_de){
  row <- gsea_res_df[gsea_res_df$ID == term_id, ][1, ]
...
core_pos <- pick_core(top_pos$ID[1], gsea_res, res_early_mature)
core_neg <- pick_core(top_neg$ID[1], gsea_res, res_early_mature)

head(core_pos, 15)
head(core_neg, 15)
# choosing my top candidates
